-- =====================================================
-- SIMPLE FIX FOR UUID/VARCHAR MISMATCH ISSUE
...
-- Step 6: Function to generate new transaction IDs
...
-- Step 7: Update existing transaction_id values to new format
-- Update transactions that don't start with 'F' (likely UUIDs)
UPDATE transactions 
SET transaction_id = generate_new_transaction_id()
WHERE transaction_id IS NOT NULL AND LEFT(transaction_id, 1) != 'F';

-- Update purchases that don't start with 'F' (likely UUIDs)
UPDATE purchases 
SET transaction_id = generate_new_transaction_id()
WHERE transaction_id IS NOT NULL AND LEFT(transaction_id, 1) != 'F';

-- Update other tables that don't start with 'F' (likely UUIDs)
