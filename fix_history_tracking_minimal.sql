-- Fix History Tracking - Minimal Version
-- This version avoids the view/table confusion by only dropping what exists

-- 1. First, let's see what exists and clean up properly
...
-- Drop any existing views (use DROP VIEW for views)
DROP VIEW IF EXISTS transaction_history_view CASCADE;
DROP VIEW IF EXISTS purchase_history_view CASCADE;
DROP VIEW IF EXISTS transaction_update_history CASCADE;
DROP VIEW IF EXISTS purchase_update_history CASCADE;
...
-- 10. Test the system
SELECT 
    'MINIMAL HISTORY TRACKING SYSTEM SETUP COMPLETE' as status,
