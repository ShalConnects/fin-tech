-- Test and Fix History Tracking System
-- This script will test if history tracking is working and fix any issues

-- 1. First, let's check if the tables exist
SELECT 
    'Checking tables...' as step,
    EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transaction_updates') as transaction_updates_exists,
    EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'purchase_updates') as purchase_updates_exists;

-- 2. Check if the functions exist
SELECT 
    'Checking functions...' as step,
    EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'log_transaction_update') as log_transaction_update_exists,
    EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'log_purchase_update') as log_purchase_update_exists;

-- 3. Check if the triggers exist
SELECT 
    'Checking triggers...' as step,
    EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_log_transaction_update') as trigger_log_transaction_update_exists,
    EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_log_purchase_update') as trigger_log_purchase_update_exists;

-- 4. Check if the views exist
SELECT 
    'Checking views...' as step,
    EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'transaction_update_history') as transaction_update_history_exists,
    EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'purchase_update_history') as purchase_update_history_exists;

-- 5. If any of the above checks fail, recreate the entire system
...
-- Create the transaction_updates table if it doesn't exist
...
-- Create the purchase_updates table if it doesn't exist
...
-- Create the logging function for transactions
...
-- Create the logging function for purchases
...
-- 6. Test the system by checking current records
SELECT 
    'Current records in transaction_updates:' as info,
    COUNT(*) as count
FROM transaction_updates;

SELECT 
    'Current records in purchase_updates:' as info,
    COUNT(*) as count
...
SELECT 
    ...
    'Now try editing a transaction and check the history page' as message;
