-- Fix final data type mismatch in sync test function
-- This script checks actual column types and fixes the function accordingly

-- 1. First, let's check the actual data types of the columns
SELECT 
    'Checking actual data types:' as info,
    table_name,
    column_name,
    data_type
FROM information_schema.columns 
WHERE (table_name = 'purchases' AND column_name = 'price') 
   OR (table_name = 'transactions' AND column_name = 'amount')
ORDER BY table_name, column_name;

-- 2. Fix the test function to match the actual data types
...
-- 3. Also fix the sync functions to use NUMERIC instead of REAL
...
SELECT 
    'FINAL DATA TYPE MISMATCH FIXED' as status,
    'Sync functions now use correct data types (NUMERIC)' as message;

-- 6. Run the test function
SELECT * FROM test_purchase_transaction_sync();

-- 7. Show sample of linked purchases and transactions
SELECT 
...
-- 9. Final verification - show data types
SELECT 
    'Final data types verification:' as info,
    table_name,
    column_name,
    data_type
FROM information_schema.columns 
